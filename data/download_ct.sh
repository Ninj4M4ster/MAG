...
dir="$(dirname "$(realpath "$0")")/clean_ct"
mkdir -p "$dir"

md5_filename="${md5_link##*/}"
md5_filepath="$dir/$md5_filename"
if [ ! -f "$md5_filepath" ]; then
    wget -c -O "$md5_filepath" "$md5_link"
else
    echo "$md5_filename already exists, skipping."
fi
...
for url in "${links[@]}"; do
    ...
    unzip_dir="${filepath%.zip}"
    if [ ! -d "$unzip_dir" ]; then
        echo "Unzipping $filename..."
        unzip -q "$filepath" -d "$unzip_dir"
    else
        echo "$filename already unzipped, skipping."
    fi
    if [ -f "$filepath" ]; then
        rm "$filepath"
        echo "Removed $filename after unzipping."
    fi
done
